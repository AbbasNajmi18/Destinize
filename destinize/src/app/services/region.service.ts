...
export class RegionService {
  ...
  public selectedRegion$: Observable<string> = this.selectedRegionSubject.asObservable();
  
  // Flag to track if we're on the national page
  private isNationalPageSubject = new BehaviorSubject<boolean>(false);
  public isNationalPage$: Observable<boolean> = this.isNationalPageSubject.asObservable();
  ...
  updateSelectedRegion(region: string, isFromNationalPage: boolean = false): void {
